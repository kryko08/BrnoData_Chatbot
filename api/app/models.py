...
from sqlalchemy import DateTime, ForeignKeyConstraint, PrimaryKeyConstraint, String
...
class AdkInternalMetadata(Base):
    ...
    __table_args__ = (
        PrimaryKeyConstraint('key', name='adk_internal_metadata_pkey'),
    )
...
class AppStates(Base):
    ...
    __table_args__ = (
        PrimaryKeyConstraint('app_name', name='app_states_pkey'),
    )
...
class Events(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['app_name', 'user_id', 'session_id'], ['sessions.app_name', 'sessions.user_id', 'sessions.id'], ondelete='CASCADE', name='events_app_name_user_id_session_id_fkey'),
        PrimaryKeyConstraint('id', 'app_name', 'user_id', 'session_id', name='events_pkey')
    )
